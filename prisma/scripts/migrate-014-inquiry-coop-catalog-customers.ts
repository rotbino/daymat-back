...
// ✅ عضویت متقابل کاتالوگ — backfill: همهٔ همکاری‌های «فعال» بازوی خرید که خریدارشان
//    هنوز در «اعضای کاتالوگ» تامین‌کننده (لِین خریدار) ثبت نشده را یک‌جا اصلاح می‌کند.
//    (رفع باگ گزارش مالک: تایید درخواست همکاری، تامین‌کننده را در اعضای بازو آورد ولی
//    خریدار در اعضای کاتالوگ تامین‌کننده نیامده بود — از این به بعد خودکار sync می‌شود)
...
async function main() {
    ...
    for (const m of members) {
        ...
        // کاتالوگِ کسب‌وکارِ خودِ خریدار نمی‌تواند خریدار خودش باشد
        if (
            inquiry.businessId &&
            catalog.businessId &&
