...
// صفحه کاتالوگ خرید (استعلام قیمت) — DTO ها
...
export class CreateInquiryDto {
    @ApiProperty({ example: 'لیست خرید هفتگی سوپرمارکت', description: 'عنوان صفحه کاتالوگ خرید' })
    ...
    @ApiPropertyOptional({ type: [InquiryUnitDto], description: 'واحدهای اختصاصی این صفحه کاتالوگ خرید (از مرجع واحد)', required: false })
    @IsOptional()
    @IsArray()
    @ArrayMaxSize(50)
    @ValidateNested({ each: true })
...
/** 🪪 ذخیره/حذف مشخصات کارت ویزیت صفحه کاتالوگ خرید (JSON) — قرینهٔ کاتالوگ فروش */
