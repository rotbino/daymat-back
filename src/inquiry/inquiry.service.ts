...
export class InquiryService implements OnModuleInit {
    ...
    async onModuleInit() {
        ...
        try {
            ...
            if (orphans.length > 0) {
                ...
                if (attached > 0) {
                    console.log(`[inquiry] ${attached} صفحه خریدِ بدون کسب‌وکار به کسب‌وکارِ یگانهٔ مالک وصل شد`);
                }
            }
        } catch (e: any) {
        ...
        try {
            ...
            if (legacy.length > 0) {
                console.log(`[inquiry] اقلام ${legacy.length} صفحه خرید قدیمی «اعلام خرید فعال» گرفتند (رفتار قبل حفظ شد)`);
            }
        } catch (e: any) {
    ...
    /** آیا این کاربر (با یکی از کاتالوگ‌های فروشش) تامین‌کنندهٔ تاییدشدهٔ این صفحه خرید است؟ */
    ...
    private async notifyUrgentAnnounce(
        ...
        try {
            ...
            void this.notification.notify({
                ...
                body: `«${itemName}» — در صفحه خرید ${inquiry.title}`,
    ...
    async findByIdOrSlug(idOrSlug: string, userId?: string) {
        ...
        if (!inquiry || inquiry.status === 'archived') {
            throw new NotFoundException({ errorCode: 'INQUIRY_NOT_FOUND', message: 'صفحه خرید پیدا نشد' });
        }
    ...
    async update(id: string, userId: string, dto: UpdateInquiryDto) {
        ...
        if (!inquiry) throw new NotFoundException({ errorCode: 'INQUIRY_NOT_FOUND', message: 'صفحه خرید پیدا نشد' });
    ...
    async remove(id: string, userId: string) {
        ...
        return { message: 'صفحه خرید حذف شد' };
    }
    ...
    private async assertOwner(id: string, userId: string) {
        ...
        if (!inquiry) throw new NotFoundException({ errorCode: 'INQUIRY_NOT_FOUND', message: 'صفحه خرید پیدا نشد' });
        if (inquiry.ownerUserId !== userId) {
            throw new ForbiddenException({ errorCode: 'FORBIDDEN', message: 'فقط صاحب صفحه خرید می‌تواند اقلام را مدیریت کند' });
        }
    }
    ...
    async addOffer(inquiryId: string, userId: string, dto: CreateOfferDto) {
        ...
        if (inquiry.ownerUserId === userId) {
            throw new BadRequestException({ errorCode: 'OWN_INQUIRY', message: 'روی صفحه خرید خودتان نمی‌توانید پیشنهاد بدهید' });
        }
        ...
        if (inquiry.visibility === 'private' && !(await this.isActiveMember(inquiryId, userId))) {
            throw new ForbiddenException({
                ...
                message: 'این صفحه خرید خصوصی است — فقط تامین‌کننده‌های تاییدشده می‌توانند قیمت بدهند',
            });
        }
    ...
    async getOffers(inquiryId: string, userId: string) {
        ...
        if (!inquiry) throw new NotFoundException({ errorCode: 'INQUIRY_NOT_FOUND', message: 'صفحه خرید پیدا نشد' });
        if (inquiry.ownerUserId !== userId) {
            throw new ForbiddenException({ errorCode: 'FORBIDDEN', message: 'فقط صاحب صفحه خرید پیشنهادها را می‌بیند' });
        }
        const offers = await this.prisma.inquiryOffer.findMany({
    ...
    async updateOffer(offerId: string, userId: string, dto: UpdateOfferDto) {
        ...
        if (dto.status === 'withdrawn') {
            ...
            if (!inquiry || inquiry.ownerUserId !== userId) {
                throw new ForbiddenException({ errorCode: 'FORBIDDEN', message: 'فقط صاحب صفحه خرید می‌تواند تصمیم بگیرد' });
            }
        }
        ...
        if (dto.status === 'withdrawn') {
            ...
            void this.notification.notify({
                ...
                body: inquiry.title ? `صفحه خرید «${inquiry.title}»` : undefined,
                actorUserId: userId,
    ...
    //    همهٔ ارتباطات کاتالوگ‌به‌کاتالوگ می‌شود: خریدار از روی صفحه خریدش
    //    تامین‌کننده اضافه می‌کند؛ تامین‌کننده اعلام خریدهای فوری را در
    //    پنل کاتالوگ فروشش می‌بیند و قیمت می‌دهد.
    // ═══════════════════════════════════════════════════════════════════

    /** فهرست تامین‌کننده‌های این صفحه خرید (مالک — تب «تامین‌کنندگان» پنل خرید) */
    ...
    async addMember(inquiryId: string, userId: string, dto: AddInquiryMemberDto) {
        ...
        void this.notification.notify({
            ...
            body: `صفحه خرید «${inquiry.title}» تو را به‌عنوان تامین‌کننده دعوت کرده`,
            actorUserId: userId,
    ...
    async requestAccess(inquiryId: string, userId: string, dto: RequestInquiryAccessDto) {
        ...
        if (inquiry.ownerUserId === userId) {
            throw new BadRequestException({ errorCode: 'OWN_INQUIRY', message: 'این صفحه خرید مال خودتان است' });
        }
        if (!this.isValidObjectId(dto.catalogId)) {
            throw new BadRequestException({ errorCode: 'INVALID_CATALOG', message: 'کاتالوگ نامعتبر است' });
        }
        ...
        void this.notification.notify({
            ...
            body: `«${catalog.name}» درخواست عضویت در صفحه خرید «${inquiry.title}» را دارد`,
            actorUserId: userId,
    ...
    async decideMember(inquiryId: string, memberId: string, userId: string, status: 'active' | 'declined' | 'removed') {
        ...
        if (member.via === 'supplier_request') {
            void this.notification.notify({
                ...
                body: `در صفحه خرید «${inquiry.title}» تایید شدید — اعلام خریدهایش را می‌بینید`,
                actorUserId: userId,
                href: '/my-catalogs?tab=leads',
                businessId: inquiry.businessId ?? null,
            });
